...
[composite action](action.yml): CLI installation,
...
```sh
python3 code-review/scripts/onboard-cursor-review.py --repo sonic-molex/TARGET_REPOSITORY
```
...
python3 code-review/scripts/onboard-cursor-review.py --repo sonic-molex/TARGET_REPOSITORY --dry-run
...
python3 code-review/scripts/onboard-cursor-review.py --repos-file /path/to/review-repos.txt
...
The default shared reference is `sonic-molex/ci/code-review@main`.
...
```sh
python3 code-review/scripts/onboard-cursor-review.py --rotate-secrets \
...
```sh
python3 -m unittest discover -s code-review/scripts/tests -v
bash -n code-review/review.sh
```

## Repository boundaries and rollout

- `sonic-molex/ci/code-review/`: shared action, review script/prompt, onboarding and
...
Changes to the shared action are tested in this repository's `Validate code review` workflow. Consumer repositories following `@main` receive changes only
